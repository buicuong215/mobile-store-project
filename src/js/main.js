  ...
  if (location === "sign") {
    ...
    btnLogin.addEventListener("click", e => {
      if (account.value === "" || password === "") {
        alert("Mời bạn nhập thông tin đăng nhập");
        return;
      }
      if (localAccount) {
    ...
    btnSignUp.addEventListener("click", e => {
      if (accountSingUp.value.length === 0 || passwordSingUp.value.length === 0 || confirmPasswordSingUp.value.length === 0) {
        alert("Bạn không được để trống thông tin sign up.");
        return;
      }
...
const btnLinkHeader = document.querySelectorAll('.header-service');

btnLinkHeader.forEach((btn) => {
...
const login = () => {
   document.querySelectorAll(".header-service")[5].classList.add("no-login");
   document.querySelectorAll(".header-service")[6].classList.remove("no-login");
   document.querySelector(".name-user").innerHTML = `${state.user}`;
   document.querySelector(".sign-container").innerHTML = `<div class="login-success">
          <h1>Đăng nhập thành công!</h1>
          <p>Chào mừng ${state.user} đến với mobile store.</p>
           <button class="btn-sign-out opacity">Sign Out</button>
          </div > `;

   document.querySelector(".btn-sign-out").addEventListener("click", e => {
     signOut();
   });
}
...
const signOut = () => {
   state.user = null;
   state.signOut = true;
   document.querySelectorAll(".header-service")[5].classList.remove("no-login");
   document.querySelectorAll(".header-service")[6].classList.add("no-login");
   reRender("#sign");
}